...
<template>
   ...
   <div class="container">
      <div class="d-flex flex-wrap justify-content-center my-2 py-3">
         <div v-for="tipo in types" class="mx-2 rounded-3 mb-3">
            <input :id="tipo.id" type="checkbox" class="checkbox d-none" :value="tipo.id" name="type_id[]">
            <label :id="'label' + tipo.id" :for="tipo.id" class="carta bg-white text-center rounded-5"
               @click="check(tipo.id), getRestaurants()">
               <img class="my-select-img" :src="`${this.store.baseUrl}storage/${tipo.image}`"
                  :alt="'Immagine ristorante ' + tipo.name">
               <h4 class="p-2 centrato flex-grow-1">{{ tipo.name }}</h4>
            </label>
         </div>
      </div>
      <div class="d-flex justify-content-center">
         <button class="fancy-button btn btn-primary p-2 fs-5" @click="reset">Reset</button>
      </div>
      <div id="lista" class="p-4 rounded-3 accordion-collapse collapse">
         ...
         <router-link :to="`/ristorante/${restaurant.slug}`" class="my-card-restaurant card mb-3 text-decoration-none"
            v-for="restaurant in restaurants">
            <div class="row h-100">

               <div class="card-containers col-12 col-sm-3">
                  <img v-if="restaurant.image" :src="`${this.store.baseUrl}storage/${restaurant.image}`"
                     class="h-100 w-100 object-fit-cover rounded-start" :alt="restaurant.name">
                  <img v-else src="/placeholder.png" class="h-100 w-100 object-fit-cover rounded-start"
                     :alt="restaurant.name">
               </div>
               ...
               <div class="card-containers col-12 col-sm-9 p-3">
                  ...
                  <span class="me-2 badge rounded-pill text-bg-warning p-2 d-none d-md-inline-block mb-1"
                     v-for="tipo in restaurant.types">{{ tipo.name }}</span>

                  <div v-if="restaurant.types.length < 3" class="overflow-x-scroll d-flex d-md-none">
                     <span class="me-2 badge rounded-pill text-bg-warning p-2" v-for="tipo in restaurant.types">{{
                        tipo.name }}</span>
                  </div>
                  <div v-else class="d-md-none">
                     <span class="me-2 badge rounded-pill text-bg-warning p-2" v-for="n in 2">{{ restaurant.types[n].name
                     }}</span>

                     <span class="dropdown">
                        <button class="me-2 badge rounded-pill text-bg-warning p-2 border-0" type="button"
                           data-bs-toggle="dropdown" aria-expanded="false">
                           +
                        </button>
                        <ul class="dropdown-menu">
                           <li v-for="tipo in restaurant.types.slice(3)" class="text-center mb-1"><span
                                 class="me-2 badge rounded-pill text-bg-warning p-2 px-4">{{ tipo.name }}</span></li>
                        </ul>
                     </span>
                  </div>
                  <p class="card-text d-sm-none">{{ truncateDescription(restaurant.description, 30) }}</p>
                  <p v-if="restaurant.description" class="card-text d-none d-sm-block">{{
                     truncateDescription(restaurant.description, 250) }}</p>
                  <!-- </div> -->
